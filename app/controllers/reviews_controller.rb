...
class ReviewsController < ApplicationController
    ...
    def new
        # movie_id because of nested route
        @movie = Movie.find(params[:movie_id])
        previous_review = Review.where(:movie_id => params[:movie_id], :moviegoer_id => @current_user.id).first
        if previous_review
    ...
    def edit
        @movie = Movie.find(params[:movie_id])
        @review = Review.where(:movie_id => params[:movie_id], :moviegoer_id => @current_user.id).first
        unless @review
